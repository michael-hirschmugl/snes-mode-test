...
; - Mode 0 (all four BGs are 2bpp)
; - BG1 only (2bpp tiles, 4-color palette)
; - Shows one 16x16 character made from four 2bpp 8x8 tiles
;
; NOTE: All BG assets in this demo are 2bpp only. In Mode 0, every BG layer
; uses 2bpp tiles and a 4-color palette slot (BG1 -> palettes 0..7 at
; CGRAM 0..31). Only palette 0 (4 colors, 8 bytes) is uploaded here.
; ------------------------------------------------------------
...
Reset:
    ...
    ; PPU setup for Mode 0 (all BGs 2bpp), BG1 only.
    stz BGMODE
    ...
    sta BG1SC
    stz BG12NBA         ; BG1 2bpp tile base @ VRAM word $0000
    stz BG1HOFS
    stz BG1HOFS
    stz BG1VOFS
    stz BG1VOFS

    ; Upload BG1 palette 0 (2bpp = one 4-color palette = 8 bytes).
    ; In Mode 0 BG1's palettes occupy CGRAM $00..$1F (palettes 0..7).
    stz CGADD
    lda #$00
    sta DMA0CTRL
    lda #$22            ; CGDATA
    ...
    ; Upload 18 BG1 tiles in 2bpp format (16 bytes per tile).
    ...
    ; to 0. Zero them explicitly to prevent window-masking or colour-math
    ; from producing a black screen on first power-on.
...
PaletteData:
    .incbin "build/mode0_2bpp/palette.bin"

TileData:
    .incbin "build/mode0_2bpp/tiles.2bpp.chr"

TilemapData:
    .incbin "build/mode0_2bpp/tilemap.bin"

.segment "HEADER"
    ; 21-byte internal title
    .byte "MODE0 16X16 PAL DEMO "
